    ...
    def input(_event, _posteventcallback):
        ...
        if _event.type == COLLISION_EVENT:
            ...
            if entity.artifacts[TagArtifact.NAME].type == TagType.KOMESMAN:
                _posteventcallback(
                    pygame.event.Event(ENTITY_EFFECT_EVENT
                                       , effect=EntityEffect.PLAY_SOUND
                                       , path="res/sound/snort.wav"))
                _posteventcallback(
                    pygame.event.Event(SCREEN_EFFECT_EVENT
                                       , type=ScreenEffectEvent.COLOR_EXPLOSION, time=4000,
